...
%% new for this exam:
enforceMaxDaysPerModerator:- moderator(M), maxDaysPerModerator(MAX), findall(md(M,D), day(D), L),  atMost(MAX, L), fail.
...
enforceAllDifferentDaysEvents:- allDifferentDaysEvents(L), member(E1, L), member(E2, L), E1 \= E2, day(D), writeOneClause([-ed(E1,D),-ed(E2,D)]), fail.
