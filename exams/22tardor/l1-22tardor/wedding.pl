...
%%%%%%  1. SAT Variables:
satVariable( gt(G, T) ):- guest(G), table(T).   % "guest G sits at table T."


%%%%%%  2. Clause generation:
writeClauses:-
    eachGuestInATable,
    minMaxPeopleAtTableRespected,
    sitTogetherRespected,
    cantSitTogetherRespected,
    true.
writeClauses:- told, nl, write('Error: writeClauses failed!'), nl, nl, halt.

eachGuestInATable :- guest(G), findall(gt(G,T), table(T), L), exactly(1,L), fail.
eachGuestInATable.

minMaxPeopleAtTableRespected :- table(T), minPeopleAtTable(MIN), maxPeopleAtTable(MAX),
                                findall(gt(G,T), guest(G), L), atMost(MAX, L), atLeast(MIN, L), fail.
minMaxPeopleAtTableRespected.

sitTogetherRespected :- mustSitTogether(G1, G2), table(T), writeOneClause([-gt(G1,T), gt(G2,T)]), fail.
sitTogetherRespected.

cantSitTogetherRespected :- cantSitTogether(G1, G2), table(T), writeOneClause([-gt(G1,T), -gt(G2,T)]), fail.
cantSitTogetherRespected.
