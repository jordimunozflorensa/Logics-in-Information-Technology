...
writeClauses:-
    oneMemberPerFamily,
    atLeastOneAtMostTwoEachAge,
    eachAgeEachSameAnimal,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

oneMemberPerFamily :- family(F), findall(sel(P), personInfo(P,_,F,_), L), exactly(1, L), fail.
oneMemberPerFamily.

atLeastOneAtMostTwoEachAge :- age(A), findall(sel(P), personInfo(P,A,_,_), L), atLeast(1, L), atMost(2, L), fail.
atLeastOneAtMostTwoEachAge.

eachAgeEachSameAnimal :- personInfo(P1,A,_,M1), personInfo(P2,A,_,M2), M1 \= M2, writeOneClause([-sel(P1), -sel(P2)]), fail.
eachAgeEachSameAnimal.
