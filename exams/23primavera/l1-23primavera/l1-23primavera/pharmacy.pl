...
% a) there must be at least MinPh pharmacies on duty every night
minimunOnDutyPharmaciesEachNight:- night(N), minPharmaciesOnDuty(MIN), findall(phn(P,N), pharmacy(P), L), atLeast(MIN, L), fail.
minimunOnDutyPharmaciesEachNight.

% b) each pharmacy has a list of nights when it cannot be on duty
blockedNights:- pharmacyBlockedNight(P,N), writeOneClause([-phn(P,N)]), fail.
blockedNights.

% c) no pharmacy can be on duty three consecutive nights
noThreeConsecutiveNights:- pharmacy(P), threeConsecutiveNights(N1,N2,N3), writeOneClause([-phm(P,N1), -phm(P,N2), -phm(P,N3)]), fail.
noThreeConsecutiveNights.

% d) no pharmacy can do more than MaxN nights on duty in the total of N nights
maxOnDutyNights:- pharmacy(P), maxNightsOnDuty(MAX), findall(phn(P,N), night(N), L), atMost(MAX, L), fail.
maxOnDutyNights.
    
% e) pharmacies in the same city cannot be on duty in the same night
noTwoPharmaciesInSameCity:- night(N), pharmaciesInSameCity(P1,P2), writeOneClause([-phn(P1,N), -phn(P2,N)]), fail.
noTwoPharmaciesInSameCity.

% f) every night there must be some on-duty pharmacy less than K Km from any city
nearbyOnDutyPharmacies:- night(N), pharmacy(P), nearbyPharmacies(P, LP), findall(phn(P1,N), member(P1, LP), L), writeOneClause([phn(P,N)|L]), fail.
