...
exactlyOneTripPerBusAndDay :- day(D), bus(B), findall(bdcc(B,D,C1,C2), trip(C1-C2), L), exactly(1, L), fail.
exactlyOneTripPerBusAndDay.

oneTrajectAtMost1PerDay :- day(D), trip(C1-C2), findall(bdcc(B,D,C1,C2), bus(B), L), atMost(1, L), fail.
oneTrajectAtMost1PerDay.

respectDepart :- bus(B), consecutiveDays(D1, D2), trip(C1-C2), trip(C3-C4), C3 \= C2, writeOneClause([-bdcc(B, D1, C1, C2),-bdcc(B, D2, C3, C4)]), fail.
respectDepart.

maxkmsRespected :- bus(B), consecutiveDays(D1, D2), tooLongDist(C1-C2-C3), writeOneClause([-bdcc(B, D1, C1, C2), -bdcc(B, D2, C2, C3)]), fail.
maxkmsRespected.

trajectAtLeastFDays :- frequency(C1-C2, MAX), findall(td(C1,C2,D), day(D), L), atLeast(MAX,L), fail.
trajectAtLeastFDays.

relateVariables :- trip(C1-C2), day(D), findall(bdcc(B,D,C1,C2), bus(B), L), expressOr(td(C1,C2,D), L), fail.
relateVariables.
