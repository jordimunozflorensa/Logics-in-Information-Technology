...
incompatibleWorkers(W1,W2) :- maxScore(MAX), workerScore(W1, S1), workerScore(W2, S2), S1+S2 > MAX.
...
respectMinSize :- team(T), minSize(MIN), findall(wt(W,T), worker(W), L), atLeast(MIN, L), fail.
respectMinSize.

respectMaxSize :- team(T), maxSize(MAX), findall(wt(W,T), worker(W), L), atMost(MAX, L), fail.
respectMaxSize.

compatibleWorkers :- worker(W1), worker(W2), W1 < W2, incompatibleWorkers(W1, W2), team(T), writeOneClause([-wt(W1, T), -wt(W2, T)]), fail.
compatibleWorkers.
