...
writeClauses :-  
    dayInHisListOfPossibleDays,  
    moderatorInHisListOfPossibleModerators,
    eachEventInOneDay,
    eachEventOneModerator,
    maximumEventsPerDay,
    maximumDaysPerModerator,
    relateEDwithEMwithMD,
    true,!.
writeClauses :- told, nl, write('writeClauses failed!'), nl,nl, halt.

dayInHisListOfPossibleDays :- eventDays(E,DAYS), day(D), \+ member(D, DAYS), writeOneClause([-ed(E,D)]), fail.
dayInHisListOfPossibleDays.

moderatorInHisListOfPossibleModerators :- eventModerators(E,MODS), moderator(M), \+ member(M, MODS), writeOneClause([-em(E,M)]), fail.
moderatorInHisListOfPossibleModerators.

eachEventInOneDay :- event(E), findall(ed(E,D), day(D), L), exactly(1, L), fail.
eachEventInOneDay.

eachEventOneModerator :- event(E), findall(em(E,M), moderator(M), L), exactly(1, L), fail.
eachEventOneModerator.

maximumEventsPerDay :- day(D), maxEventsPerDay(MAX), findall(ed(E,D), event(E), L), atMost(MAX, L), fail.
maximumEventsPerDay.

maximumDaysPerModerator :- moderator(M), maxDaysPerModerator(MAX), findall(md(M,D), day(D), L), atMost(MAX, L), fail.
maximumDaysPerModerator.

relateEDwithEMwithMD :- event(E), moderator(M), day(D), writeOneClause([-ed(E,D), -em(E,M), md(M,D)]), fail.
relateEDwithEMwithMD.
