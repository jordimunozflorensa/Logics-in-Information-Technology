...
%4: Escrivim el resultat:
        pretty_print(RowSums, ColSums, MatrixByRows), halt.
...
matrixByRows([], _, []).
matrixByRows(L, NumCols, [Row|LRows]) :-
        append(Row, Rest, L),
        length(Row, NumCols),
        matrixByRows(Rest, NumCols, LRows).
        
declareConstraints([], []).
declareConstraints([RS|RSS], [MRS|MRSs]) :-
        sum(MRS, #=, RS),
        declareConstraints(RSS, MRSs).
