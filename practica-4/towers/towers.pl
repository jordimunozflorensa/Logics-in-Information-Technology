...
writeClauses(MaxNumTowers):-
        ...
        eachVillageWatched,
	relateTowerPosWithTowerVil, % towerVil(a) <-> expressor[towerPos(2,20),towerPos(2,21),towerPos(3,20),towerPos(3,21)]
	maxTowers(MaxNumTowers),
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

maxOneTowerPerVillage :- village(V), 
                         findall(towerPos(I,J), posVillage(V, I, J), L),
                         atMost(1, L),
                         fail.
maxOneTowerPerVillage.

eachTowerInAVillage :- position(I, J), 
                       not(posVillage(_, I, J)),
                       writeOneClause([-towerPos(I, J)]),
                       fail.
eachTowerInAVillage.

eachSignificantVillageOneTower :- significantVillage(V),
                                  writeOneClause([towerVillage(V)]),
                                  fail.
eachSignificantVillageOneTower.

eachVillageWatched :- village(V),
                      findall(towerPos(I, J), posWatchesVillage(V, I, J), L),
                      atLeast(1, L),
                      fail.
eachVillageWatched.

relateTowerPosWithTowerVil:- village(V),
                             findall(towerPos(I,J), posVillage(V,I,J), L),
                             expressOr(towerVillage(V), L),
                             fail.
relateTowerPosWithTowerVil.

maxTowers(MaxNumTowers):- upperLimitTowers(MAX),
                          Max is min(MaxNumTowers, MAX),
