...
twoConsecutiveHours(H1,H2) :- hour(H1), H2 is H1+1, hour(H2).
...
writeClauses(MaxConsecutiveHours) :-
    gangsterNotAvailableInHour,
    minimumNumGangstersPerTaskPerHour,
    eachGangsterHourOnlyOneTask,
    notTwoTasksInTwoConsecutiveHours,
    relateVariables,
    maxConsecutiveHoursPerGangster(MaxConsecutiveHours),
    true,!.
writeClauses(_) :- told, nl, write('writeClauses failed!'), nl,nl, halt.

gangsterNotAvailableInHour :- blocked(G,H), writeOneClause([-gh(G,H)]), fail.
gangsterNotAvailableInHour.

minimumNumGangstersPerTaskPerHour :- needed(T,H,N), findall(does(G,T,H), gangster(G), L), exactly(N,L), fail.
minimumNumGangstersPerTaskPerHour.

eachGangsterHourOnlyOneTask :- gangster(G), hour(H), findall(does(G,T,H), task(T), L), atMost(1, L), fail.
eachGangsterHourOnlyOneTask.

notTwoTasksInTwoConsecutiveHours :- gangster(G),
                                    twoConsecutiveHours(H1, H2),
                                    task(T1), task(T2), T1 \= T2,
                                    writeOneClause([-does(G,T1,H1), -does(G,T2,H2)]), fail.
notTwoTasksInTwoConsecutiveHours.

relateVariables :- gangster(G), hour(H),
                   findall(does(G,T,H), task(T), L),
                   expressOr(gh(G,H), L), fail.
relateVariables.

maxConsecutiveHoursPerGangster(MaxConsecutiveHours) :-  gangster(G),
                                                        hour(H1), hour(H2),
                                                        H2 is H1 + MaxConsecutiveHours,                                                        
                                                        findall(-gh(G,H), between(H1,H2,H), Lits),
                                                        writeOneClause(Lits), 
                                                        fail.
maxConsecutiveHoursPerGangster(_).
...
costOfThisSolution(M,Cost):- 
   ...
   append(L1,_,L), append(_,Int,L1),
   length(Int, Cost),
   first(Int, F), last(Int, U),     
   U is F+Cost-1.
